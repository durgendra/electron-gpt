...
const {
  getMessageHistoryOrCreateMessage,
  updateConversation,
  getConversations,
...
async function generateText(prompt, conversationId) {
  try {
    const { messageHistory, conversationId: newConversationId } =
      await getMessageHistoryOrCreateMessage(conversationId, prompt);
    ...
    const completion = await openai.createChatCompletion({
      ...
      messages: [
        {
          role: "system",
          content: "You are a helpful assistant. You speak on French language.",
        },
        ...messageHistory,
      ],
    });
